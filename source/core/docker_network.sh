...
#      --aux-address map      Auxiliary IPv4 or IPv6 addresses used by Network driver (default map[])
  ...
  [ ! "$gateway" ] || {
    # dc::argument::check gateway "$DC_TYPE_IPV4" || dc::argument::check aux_address "$DC_TYPE_IPv6" || return
  ...
  [ ! "$aux_address" ] || {
    # dc::argument::check aux_address "$DC_TYPE_IPV4" || dc::argument::check aux_address "$DC_TYPE_IPv6" || return
    while read -r sub; do
      com+=(--aux-address "$sub")
    done <<<"$aux_address"
  }
